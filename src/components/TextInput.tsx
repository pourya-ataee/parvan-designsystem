...
interface IProps extends InputHTMLAttributes<HTMLInputElement> {
	...
	/**
	 * @default small
	 * Input height (large: 56px | small: 48px)
	 */
	height?: 'small' | 'large';
	/**
	 * Input width in px
...
const TextInputC = (props: TextInputProps) => {
	...
	return (
		<div className={className}>
			<span
				className={`parvan-input-container status-${!!disabled ? 'disabled' : !!error ? 'error' : focus ? 'focus' : 'enable'} ${
					isVisible ? 'has-value' : ''
				} ${!!error && !!focus ? 'focus-with-error' : ''}`}
			>
...
export const TextInput = styled(TextInputC)`
    ...
    .parvan-input-container {
        position: relative;
        background: ${(props) =>
					clsx({ 'var(--surface-variant-color)': props.variant === 'filled' || !props.variant }, { transparent: props.variant === 'outlined' })};
        border-radius: ${(props) => clsx({ '4px 4px 0 0': props.variant === 'filled' || !props.variant }, { '4px': props.variant === 'outlined' })};
        .input-icon {
            position: absolute;
            z-index: 2;
            top: 50%;
        ...
        &:before {
            ...
            right: 0;
            left: 0;
            display: ${(props) => clsx({ block: props.variant === 'filled' || !props.variant }, { 'none !important': props.variant === 'outlined' })};
            height: 1px;
        ...
        &.status-focus {
            ...
            .parvan-input {
                border: ${(props) =>
									clsx({ '0': props.variant === 'filled' || !props.variant }, { '2px solid var(--primary-color)': props.variant === 'outlined' })};
            }
        }
        &:hover {
            &:before {
        ...
        &.status-error {
            ...
            .parvan-input {
                border: ${(props) =>
									clsx({ '0': props.variant === 'filled' || !props.variant }, { '2px solid var(--error-color)': props.variant === 'outlined' })};
            }
        }
        &.focus-with-error {
            .parvan-label {
        ...
        &.status-disabled {
            ...
            .parvan-input {
                background: ${(props) =>
									clsx({ '#F6F6F6 !important': props.variant === 'filled' || !props.variant }, { transparent: props.variant === 'outlined' })};
                color: #A9A8AA !important;
                border: ${(props) =>
									clsx({ '0': props.variant === 'filled' || !props.variant }, { '1px solid #F6F6F6': props.variant === 'outlined' })};
            }
        }
    }
    .supporting-text {
    ...
    .parvan-label {
        position: absolute;
        z-index: 2;
        right: ${(props) => clsx({ '16px': !props.icon }, { '48px': !!props.icon })};
        top: ${(props) =>
					clsx(
						{ '8px': (props.variant === 'filled' || !props.variant) && props.height === 'large' },
						{ '4px': (props.variant === 'filled' || !props.variant) && (props.height === 'small' || !props.height) },
						{ '50%': props.variant === 'outlined' }
					)};
        transform: ${(props) => clsx({ none: props.variant === 'filled' || !props.variant }, { 'translateY(-50%)': props.variant === 'outlined' })};
        font-size: ${(props) =>
					clsx(
    ...
    .parvan-input {
        border: ${(props) =>
					clsx({ '0': props.variant === 'filled' || !props.variant }, { '1px solid var(--outline-color)': props.variant === 'outlined' })};
        position: relative;
        z-index: ${(props) => clsx({ '1': props.variant === 'filled' || !props.variant }, { '2': props.variant === 'outlined' })};
        transition: 0.3s;
        border-radius: ${(props) => clsx({ '4px 4px 0 0': props.variant === 'filled' || !props.variant }, { '4px': props.variant === 'outlined' })};
        background: transparent;
        width: ${(props) => clsx({ '210px': !props.width }, { [`${props.width}px`]: !!props.width })};
        height: ${(props) => clsx({ '48px': props.height === 'small' || !props.height }, { '56px': props.height === 'large' })};
        ...
        padding: ${(props) =>
					clsx(
						...
						{ '24px 48px 8px': !!props.label && !!props.icon && props.variant !== 'outlined' && props.height === 'large' },
						{ '20px 48px 4px': !!props.label && !!props.icon && props.variant !== 'outlined' && (props.height === 'small' || !props.height) },
						{ '8px 16px 8px 48px': !props.label && !props.icon && props.variant !== 'outlined' },
						{ '24px 16px 8px 48px': !!props.label && !props.icon && props.variant !== 'outlined' && props.height === 'large' },
						{ '20px 16px 4px 48px': !!props.label && !props.icon && props.variant !== 'outlined' && (props.height === 'small' || !props.height) },
