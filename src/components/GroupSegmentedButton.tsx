...
interface SegmentedButtonPropsWithIds extends SegmentedButtonProps {
	id: string;
}
...
const GroupSegmentedButtonC = (props: GroupSegmentedButtonProps) => {
	...
	return (
		<div
			className={`${className} group-segmented-button-container group-segmented-button-dir-${!!dir ? dir : 'ltr'}`}
			{...other}
		>
			{propList.map((e) => (
				<SegmentedButton
					key={e.id}
					{...e}
					selected={!multiple ? selected === e.id : multipleSelect.includes(e.id)}
